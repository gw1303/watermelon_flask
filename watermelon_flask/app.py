...
def getSongId():
    ...
    # 입력받은 가수와 제목으로 df 구성
    findArtistDf = songDf[songDf.artist_name_basket.str.contains(
        artist.strip())].sort_values(by='song_name')

    if len(findArtistDf.song_name.str.replace(' ', '').str.contains(song.strip())) > 0:
        findSongDf = findArtistDf[findArtistDf.song_name.str.replace(
            ' ', '').str.contains(song.strip())]
    else:
   ...
   # 검색된 노래가 하나일 경우
        ...
        print("\n%s 의 ['%s'] 곡이 추가되었습니다." % (
            findSongDf.artist_name_basket.tolist()[0], findSongDf.song_name.tolist()[0]))
